/* global betinhoustonIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'betinhouston-supports-dark-theme' ) ) {
	betinhoustonDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function betinhoustonDarkModeEditorInit( attempt ) {
	...
	if ( betinhoustonIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						betinhoustonDarkModeEditorInit( attempt + 1 );
					},
